...
describe("Use case: Registration Flow (all successful)", () => {
    ...
    const createUserResponse = await fetch(
      "http://localhost:3000/api/v1/users",
      {
        ...
        body: JSON.stringify({
          ...
          email: "registration.flow@felipebatista.dev",
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@felipebatista.dev>");
    expect(lastEmail.recipients[0]).toBe("<registration.flow@felipebatista.dev>");
